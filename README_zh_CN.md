...
- 写入 `include-android-user: 0` 的安卓用户路由，并排除 root UID `0` 避免代理自环；
- 重启后会等待 Android 用户数据解锁，直到 App 配置可用再补启动；
- 默认使用 gVisor TUN 栈，避免 root 创建 TUN 时 Android App TCP 流量卡在 `SYN-SENT`；
